...
for mode in "${MODES[@]}"; do
  for group in "${NODE_GROUPS[@]}"; do
    ...
    job_name="dynampi_small_${mode}_$(echo "$group" | tr ' ' '_')"
    job_script="#!/bin/bash
...
run_one() {
  ...
  echo \"##### dist=\$dist fanout=\$fanout nodes=\$nodes mode=${mode} expected_us=\$us duration_s=20 #####\"
  timeout 90 mpiexec -n \"\$ranks\" --ppn 102 \"\$SS\" --distribution \"\$dist\" --mode ${mode} \\
...
echo "SMALL-GROUP SUBMISSION DONE"

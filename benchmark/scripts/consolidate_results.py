...
Every measurement of a configuration is kept, repeats included. Reducing
them here was redundant: the plot scripts already do it themselves, in
``plot_common.dedupe_newest``, and ``plot_common.iter_csv_rows`` reads the
per-row ``job_id`` this script writes so that the reduction stays per-row once
many jobs share one file. Dropping repeats therefore discarded data no figure
needed dropped. Pass ``--latest-only`` for the old one-row-per-configuration
output.

Rows a sweep's plot filter rejects outright are still excluded; see
``is_plottable``. That is a validity filter rather than de-duplication.

For repeat and provenance analysis, ``--all-runs-dir`` additionally writes one
audit CSV per sweep. It retains every source row, excluded ones included, and
records how each was classified.
"""
...
def build_consolidation(
    results_dir: str, sweep: str, keep_duplicates: bool = True
) -> Consolidation:
    """Read every source row and mark which ones reach the consolidated CSV.

    By default every plottable row is kept, repeats included. Set
    ``keep_duplicates`` false to reduce each configuration to its newest run,
    which is what this script used to do unconditionally.
    """
    ...
    for record in records:
        if not record.plottable:
            ...
            continue
        if record.key is not None and newest[record.key] is record:
            record.selected = True
            record.selection_reason = "selected_latest"
        elif keep_duplicates:
            record.selected = True
            record.selection_reason = "selected_repeat"
...
def consolidate(
    ...
    """Return the consolidated output: every repeat unless reduced."""
    result = build_consolidation(results_dir, sweep, keep_duplicates)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-dir",
        help="Directory for the consolidated CSVs (default: --results-dir)",
    )
    parser.add_argument(
        "--latest-only",
        action="store_true",
        help="Reduce each configuration to its newest run instead of keeping every "
        "repeat. The plot scripts already apply this reduction themselves, so this "
        "only makes the committed CSVs smaller.",
    )
    parser.add_argument(
        "--all-runs-dir",
    ...
    for sweep in args.sweeps:
        result = build_consolidation(results_dir, sweep, not args.latest_only)
        ...
        repeats = sum(
            record.selection_reason == "selected_repeat" for record in result.records
        )
        ...
        print(
            ...
            f"({repeats} repeats kept, {superseded} superseded, "
            f"{rejected} excluded) -> {out}"
        )
        if not args.check:
